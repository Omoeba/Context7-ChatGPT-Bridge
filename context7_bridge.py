...
# Load environment from .env (if present) before using os.getenv
...
class Context7Client:
    ...
    def _call_context7(self, tool_name: str, arguments: Dict[str, Any]) -> str:
        ...
        try:
            ...
            # Define Context7 MCP package constant
            context7_mcp_package = "@upstash/context7-mcp@latest"

            # Call Context7 MCP server - try different npx paths
            base_commands = [
                ["npx", "-y", context7_mcp_package],
                ["C:\\Program Files\\nodejs\\npx.cmd", "-y", context7_mcp_package],
                [
                    "C:\\Users\\S\\AppData\\Roaming\\npm\\npx.cmd",
                    "-y",
                    context7_mcp_package,
                ],
                ["wsl", "npx", "-y", context7_mcp_package],
            ]
            ...
            for cmd in npx_commands:
                try:
                    ...
                    if result.returncode == 0 and result.stdout:
                        break
                    last_error = (
                        f"Command {cmd[0]} failed: returncode={result.returncode}"
                    )

                except (FileNotFoundError, subprocess.TimeoutExpired) as e:
                    last_error = f"Command {cmd[0]} error: {e!s}"
                    continue
                except Exception as e:
                    last_error = f"Unexpected error with {cmd[0]}: {e!s}"
                    continue
            ...
            for line in lines:
                if line.strip():
                    try:
                        ...
                        if (
                            response.get("id") == self.request_id
            ...
            return f"Error calling Context7: {e!s}"
...
class ChatGPTContext7Bridge:
    ...
    def search(self, query: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if query.startswith("/") and "/" in query[1:]:
                ...
                if (
                    "Error calling Context7" in test_docs
                    or "No documentation found" in test_docs
                ):
                    return {"results": []}
            ...
            if (
                "No matching libraries found" in context7_response
                or "Error calling Context7" in context7_response
            ):
                return {"results": []}
            ...
            for lib in libraries:
                ...
                result = {
                    ...
                    "url": f"https://context7.com{lib['id']}"
                    if lib.get("id")
                    else None,
                }
    ...
    def fetch(self, id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Parsed fetch parameters: base_id={base_id}, topic={topic}, tokens={tokens}",
            )
            ...
            if base_id.startswith("/") and "/" in base_id[1:]:
                ...
                if base_id not in self.search_cache:
                    raise ValueError(
                        f"Unknown document ID: {base_id}. Please search first to get valid IDs.",
                    )
            ...
            raise ValueError(f"Failed to fetch document: {e!s}")
                ...
                try:
                    ...
                    if self.process.poll() is None:
                        ...
                        if self.tunnel_url:
                            ...
                            logger.info(
                                f"🔗 ChatGPT SSE endpoint: {self.tunnel_url}/sse",
                            )
                            return True
...
def main():
    ...
    logger.info("🚀 Starting ChatGPT-Compatible Context7 MCP Bridge")
    ...
    if not args.no_ngrok:
        ...
        if ngrok_started:
            ...
            logger.info("   2. Copy the https URL + /sse to ChatGPT")
    else:
